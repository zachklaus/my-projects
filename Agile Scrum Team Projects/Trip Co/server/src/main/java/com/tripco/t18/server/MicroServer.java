...
import com.tripco.t18.misc.Validator;
...
class MicroServer {
  ...
  private String processTIPrequest(Type tipType, Request request, Response response) {
    ...
    response.status(200);
    try {
      Validator.isValid(request.body(), tipType); //throws ValidationException if invalid
      Gson jsonConverter = new Gson();
      ...
      return responseBody;
    } catch(ValidationException ve){    //bad request from client. Send back 400.
